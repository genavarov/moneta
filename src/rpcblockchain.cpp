...
Object blockToJSON(const CBlock& block, const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("pow_hash", block.GetPoWHash(block.LastHeight+1).GetHex()));
    ...
    if (block.nVersion & BLOCK_VERSION_AUXPOW) {
        ...
        parent_block.push_back(Pair("pow_hash", block.auxpow->parentBlockHeader.GetPoWHash(block.auxpow->parentBlockHeader.LastHeight+1).GetHex()));
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            "settxfee <amount XMN/KB>\n"
            "<amount> is a real and is rounded to the nearest 0.00000001 XMN per KB");
